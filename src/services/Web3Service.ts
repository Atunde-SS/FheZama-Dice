...
export class Web3Service {
  ...
  async startGame(): Promise<void> {
    ...
    try {
      const playerAddress = this.accounts[0].toLowerCase();
      // Initialize starting balance
      Web3Service.playerBalances.set(playerAddress, 500);
      console.log("Game started with 500 ZAMA for player:", playerAddress);
  ...
  async rollDice(): Promise<void> {
    ...
    try {
      const playerAddress = this.accounts[0].toLowerCase();
      if (!Web3Service.playerBalances.has(playerAddress)) {
        throw new Error("Game not started");
      }

      // Simulate dice roll affecting balance
      const roll = Math.floor(Math.random() * 6) + 1;
      const currentBalance = Web3Service.playerBalances.get(playerAddress)!;
      
      // Calculate balance change based on roll
      let balanceChange = 0;
      if (roll === 6) {
        balanceChange = 100; // Bonus for rolling 6
      } else if (roll > 3) {
        balanceChange = 50; // Medium reward for 4-5
      } else {
        balanceChange = 25; // Small reward for 1-3
      }
      
      // Update player balance
      Web3Service.playerBalances.set(playerAddress, currentBalance + balanceChange);
      
      // Simulate blockchain transaction
      await this.contract.methods.rollDice().send({ from: this.accounts[0] });
      
      console.log(`Rolled ${roll}, balance changed by ${balanceChange}`);
    } catch (error: any) {
  ...
  private static playerBalances: Map<string, number> = new Map();
  ...
  async decryptBalance(encryptedBalance: string): Promise<string> {
    ...
    try {
      ...
      if (!Web3Service.playerBalances.has(playerAddress)) {
        Web3Service.playerBalances.set(playerAddress, 500); // Initial balance when game starts
      }
